...
namespace ora {
    ...
    inline bool record_within_block(const NextRecordOffset& offset) {
        return offset.skip_blocks == 0 || (offset.skip_blocks == 1 && offset.next_offset == 16);
    }
    inline bool last_record_in_block(const NextRecordOffset& offset) {
    ...
    int DefaultRecordSource::fill_Records() {
        ...
        Block block = std::move(*b0);

        auto out = 0;
        auto start = block.head.offset;

        if (block.offsets.size() == 0) {
            show(block);
        }

        for (const auto& info : block.offsets) {
            ...
            if (need > 0) {
                ...
                if (last.offsets.size() != 0 && last.head.offset != info.next_offset) {
                    show(last);
