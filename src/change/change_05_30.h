...
namespace ora {
    ...
    /// 5.30 #1 * KTUDX (KTU Distributed Txn)
    ...
    struct Ktudx {
        ...
        static Result<Ktudx> decode(tcb::span<const char> buf, bool isLittle);
    };
    ...
    struct Change_0530 {
        ...
        static Result<Change_0530> parse(SpanCursor &ctx);
    };

    // --------------------------------------------------------------------------------

    inline Result<Ktudx> Ktudx::decode(tcb::span<const char> buf, bool isLittle) {
        if (buf.size() < 24) {
    ...
    inline Result<Change_0530> Change_0530::parse( SpanCursor &ctx ) {

        Change_0530 out;

        // [# 1] udx
        auto udx = ctx.one_of<Ktudx>("Change5_30:udx", Ktudx::decode);
